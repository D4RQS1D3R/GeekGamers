...
new const
	ENT_MODELS[]	= "models/[GeekGamers]/v_tripmine.mdl",
...
public plugin_init() {
	...
	g_LCMDMODE	= register_cvar("bio_ltm_cmd_mode","2");	//0 is +USE key, 1 is bind, 2 is each.
#else
	...
	g_LCMDMODE	= register_cvar("amx_ltm_cmd_mode","2");	//0 is +USE key, 1 is bind, 2 is each.
#endif
...
{
	register_native("SetUserLaserMine", "furien_lasermine", 1);
}

public furien_lasermine(id)
{
	if(g_mine[id])
	{
		ChatColor(id, "!t[GG][Level-Menu] !nYou Can Use This Item Only !g1 Time !nPer Round.");
	}
	else
	if(!g_mine[id])
	{
	...
	if( mode == 0)
	{
		if(g_havemine[id] <= 0)
		{
			...
			if(cs_get_user_team(id) == CS_TEAM_CT)
			{
				...
				{
					ChatColor(id, "!t[GG] !nYou don't Have !gLaserMines !n!")
				}
			}
			else
			{
...
{	
	if(cs_get_user_team(id) == CS_TEAM_CT)
	{
		...
		ChatColor(id, "!t[GG] !nYou Have !g%i !nLaser Mine", g_havemine[id])
		emit_sound(id, CHAN_ITEM, ENT_SOUND5, VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
	}
	return PLUGIN_HANDLED
}
