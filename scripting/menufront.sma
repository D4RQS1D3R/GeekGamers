...
AddDefaultMenus()
{
	...
	AddMenu("UNBAN_PLAYER", "amx_unbanmenu", get_clcmd_flags("amx_unbanmenu", flags) ? flags : ADMIN_BAN, "Players Menu")
	//AddMenu("GAG_PLAYER", "amx_gagmenu", get_clcmd_flags("amx_gagmenu", flags) ? flags : ADMIN_BAN, "Players Menu")
	...
	AddMenu("CLIENT_COM", "ggsv_amx_clcmdmenu", get_clcmd_flags("ggsv_amx_clcmdmenu", flags) ? flags : ADMIN_LEVEL_A, "Players Menu")
	AddMenu("SERVER_COM", "ggsv_amx_cmdmenu", get_clcmd_flags("ggsv_amx_cmdmenu", flags) ? flags : ADMIN_MENU, "Commands Menu")
	AddMenu("CVARS_SET", "ggsv_amx_cvarmenu", get_clcmd_flags("ggsv_amx_cvarmenu", flags) ? flags : ADMIN_CVAR, "Commands Menu")
	AddMenu("CONFIG", "ggsv_amx_cfgmenu", get_clcmd_flags("ggsv_amx_cfgmenu", flags) ? flags : ADMIN_MENU, "Commands Menu")
	AddMenu("LANG_SET", "amx_langmenu", get_clcmd_flags("amx_langmenu", flags) ? flags : ADMIN_CFG, "Multi-Lingual System")
	AddMenu("STATS_SET", "ggsv_amx_statscfgmenu", get_clcmd_flags("ggsv_amx_statscfgmenu", flags) ? flags : ADMIN_CFG, "Stats Configuration")
	AddMenu("PAUSE_PLUG", "ggsv_amx_pausecfgmenu", get_clcmd_flags("ggsv_amx_pausecfgmenu", flags) ? flags : ADMIN_CFG, "Pause Plugins")
...
displayMenu(id, pos)
{
	...
	g_coloredMenus ? "\d[\yGeek~Gamers\d] \rAMX Mod X Menu:\R%d/%d^n\w^n" : "\d[\yGeek~Gamers\d] \rAMX Mod X Menu: %d/%d^n^n" , pos + 1, (g_menusNumber / MENUITEMSPERPAGE) + (((g_menusNumber % MENUITEMSPERPAGE) > 0) ? 1 : 0))
	...
	for (new a = start; a < end; ++a)
	{
			 ...
			 (is_plugin_loaded(g_menuPlugin[a], true) != -1)))	// search plugins for filename
		{
			...
			if (GetLangTransKey(g_menuBody[a]) != TransKey_Bad)
				len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%L^n", ++b, id, g_menuBody[a])
			else
				len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", ++b, g_menuBody[a])
		} else {
			++b
			...
			{
				if (GetLangTransKey(g_menuBody[a]) != TransKey_Bad)
					len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%L^n\w", b, id, g_menuBody[a])
				else
					len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%s^n\w", b, g_menuBody[a])
			} else {
				if (GetLangTransKey(g_menuBody[a]) != TransKey_Bad)
	...
	{
		format(menuBody[len], charsmax(menuBody) - len, "^n\y9. \w%L...^n\y0. \w%L", id, "MORE", id, pos ? "BACK" : "EXIT")
		keys |= MENU_KEY_9
	} else {
		format(menuBody[len], charsmax(menuBody) - len, "^n\y0. \w%L", id, pos ? "BACK" : "EXIT")
	}
...
clientDisplayMenu(id, pos)
{
	...
	new len = format(menuBody, charsmax(menuBody), g_coloredMenus ? "\d[\yGeek~Gamers\d] \rAMX Mod X Client Menu:\R%d/%d^n\w^n" : "\d[\yGeek~Gamers\d] \rAMX Mod X Client Menu: %d/%d^n^n" , pos + 1, (g_clientMenusNumber / MENUITEMSPERPAGE) + (((g_clientMenusNumber % MENUITEMSPERPAGE) > 0) ? 1 : 0))
	...
	for (new a = start; a < end; ++a)
	{
			 ...
			 (is_plugin_loaded(g_clientMenuPlugin[a], true) != -1)))		// search plugins for file name
		{
			...
			if (GetLangTransKey(g_clientMenuBody[a]) != TransKey_Bad)
				len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%L^n", ++b, id, g_clientMenuBody[a])
			else
				len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", ++b, g_clientMenuBody[a])
		} else {
			++b
			...
			{
				if (GetLangTransKey(g_clientMenuBody[a]) != TransKey_Bad)
					len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%L^n\w", b, id, g_clientMenuBody[a])
				else
					len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%s^n\w", b, g_clientMenuBody[a])
			} else {
				if (GetLangTransKey(g_clientMenuBody[a]) != TransKey_Bad)
