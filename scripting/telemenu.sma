...
displayTelMenu(id, pos)
{
	...
	{
		keys |= MENU_KEY_7
		len += formatex(menuBody[len], charsmax(menuBody)-len, "^n\y7. \rTo location:\w %.0f %.0f %.0f^n", g_menuOrigin[id][0], g_menuOrigin[id][1], g_menuOrigin[id][2])
	}
	else if (g_menuOption[id])	// -1
	{
		if (g_coloredMenus)
			len += formatex(menuBody[len], charsmax(menuBody)-len, "^n\y7. \d%L^n\w", id, "CUR_LOC")
		else
			len += formatex(menuBody[len], charsmax(menuBody)-len, "^n#. %L^n", id, "CUR_LOC")
	} else {					// 0
		keys |= MENU_KEY_7
		len += formatex(menuBody[len], charsmax(menuBody)-len, "^n\y7. \w%L^n", id, "CUR_LOC")
	}

	len += formatex(menuBody[len], charsmax(menuBody)-len, "\y8. \w%L^n", id, "SAVE_LOC")
