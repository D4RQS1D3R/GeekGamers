...
new const szLogDir[] = "addons/amxmodx/logs/GG_ChatLogger"
...
public logtext(id)
{
	...
	new timestr[9], authid[32], ip[16], country[32], cmd[9], logmsg[MAXLEN + 1]
	...
	{
		formatex(LogFile, 64, "addons/amxmodx/logs/GG_ChatLogger/%s.htm", datestr)
		...
		formatex(logmsg, MAXLEN, "%s &lt;%s&gt;&lt;%s&gt;&lt;%s&gt; [LEVEL %i]", timestr, authid, ip, country, get_level(id))
	}
	else
	{
		formatex(LogFile, 64, "%s/ChatLog.htm", szLogDir)
		if(!file_exists(LogFile))
		{
		...
		formatex(logmsg, MAXLEN, "%s - %s &lt;%s&gt;&lt;%s&gt;&lt;%s&gt; [LEVEL %i]", datestr, timestr, authid, ip, country, get_level(id))
	}
