...
public CmdSetBanTimes( )
{
	...
	for( new i = 1; i < iArgs; i++ )
	{
		...
		{
			server_print( "[GG][GAG] Time must be an integer!" );
			continue;
		}
		...
		if( iTime < 0 )
		{
			server_print( "[GG][GAG] Time must be a positive integer!" );
			continue;
		}
		
		if( 0 < iTimeLimit < iTime )
		{
			server_print( "[GG][GAG] Time more then %d is not allowed!", iTimeLimit );
			continue;
		}
	...
	if( TrieKeyExists( g_tArrayPos, g_szAuthid[ id ] ) )
	{
		...
		for( new i; i < iNum; i++ )
		{
			...
			if( get_user_flags( iPlayer ) & ADMIN_KICK )
			{
				...
				{
					GreenPrint( iPlayer, id, "^4[GG][GAG]^1 Gagged player ^"^3%s^1<^4%s^1>^" has disconnected!", szName, g_szAuthid[ id ] );
				}
				else
				{
					client_print( iPlayer, print_chat, "[GG][GAG] Gagged player ^"%s<%s>^" has disconnected!", szName, g_szAuthid[ id ] );
				}
			}
		}
	}
...
public client_infochanged( id )
{
	...
	if( !equal( szNewName, szOldName ) )
	{
		if( g_bColorSupported )
		{
			GreenPrint( id, id, "^4[GG][GAG]^1 Gagged players cannot change their names!" );
		}
		else
		{
			client_print( id, print_chat, "[GG][GAG] Gagged players cannot change their names!" );
		}
	...
	for( new i = 0; i < g_iGagged; i++ )
	{
		...
		if( data[ GAG_TIME ] > 0 && ( data[ GAG_START ] + data[ GAG_TIME ] ) <= iSystime )
		{
			...
			if( is_user_connected( id ) )
			{
				...
				if( g_bColorSupported )
				{
					GreenPrint( 0, id, "^4[GG][GAG]^1 Player ^"^3%s^1^" is no longer gagged", szName );
				}
				else
				{
					client_print( 0, print_chat, "[GG][GAG] Player ^"%s^" is no longer gagged", szName );
				}
			}
			else
			{
				if( g_bColorSupported )
				{
					GreenPrint( 0, 0, "^4[GG][GAG]^1 SteamID ^"^3%s^1^" is no longer gagged", data[ GAG_AUTHID ] );
				}
				else
				{
					client_print( 0, print_chat, "[GG][GAG] SteamID ^"%s^" is no longer gagged", data[ GAG_AUTHID ] );
				}
			}
	...
	if( TrieGetCell( g_tArrayPos, g_szAuthid[ id ], iArrayPos ) )
	{
		...
		if( data[ GAG_FLAGS ] & iFlags[ bTeam ] )
		{
			if( data[ GAG_TIME ] > 0 )
			{
				...
				if( g_bColorSupported )
				{
					GreenPrint( id, id, "^4[GG][GAG]^3 %s^1 left before your ungag!", szInfo );
				}
				else
				{
					client_print( id, print_chat, "[GG][GAG] %s left before your ungag!", szInfo );
				}
			}
			else
			{
				if( g_bColorSupported )
				{
					GreenPrint( id, id, "^4[GG][GAG]^3 You are gagged permanently!" );
				}
				else
				{
					client_print( id, print_chat, "[GG][GAG] You are gagged permanently!" );
				}
			}
...
DisplayGagMenu( const id, iPosition )
{
	...
	new iLen = formatex( szMenu, 511, g_bColorSupported ? "\d[\yGeek~Gamers\d] \rGag Menu:\R%i/%i^n^n" : "Gag Menu %i/%i^n^n", iPosition + 1, ( ( iNum + PERPAGE - 1 ) / PERPAGE ) );
	...
	for( new i = iStart; i < iEnd; ++i )
	{
		...
		if( iPlayer == id )
		{
			...
			if( g_bColorSupported )
				iLen += formatex( szMenu[ iLen ], 511 - iLen, "\y%i. \d%s \r(Self)^n", iCount, szName );
			else
				iLen += formatex( szMenu[ iLen ], 511 - iLen, "#. %s^n", szName );
		}
		else
		if( (iFlags & ADMIN_IMMUNITY) && bUngag != TrieKeyExists( g_tArrayPos, g_szAuthid[ iPlayer ] ) )
		{
			++iCount;
			
			if( g_bColorSupported )
				iLen += formatex( szMenu[ iLen ], 511 - iLen, "\y%i. \d%s \r(Flagged)^n", iCount, szName );
			else
				iLen += formatex( szMenu[ iLen ], 511 - iLen, "#. %s^n", szName );
		}
		else
		if( iPlayer != id )
		{
			...
			iLen += formatex( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "\y%i.\w %s\y %s^n" : "%i. %s%s%s^n", iCount, szName, TrieKeyExists( g_tArrayPos, g_szAuthid[ iPlayer ] ) ? "[GAGGED]" : "" );
		}
	}
	...
	iLen += formatex( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? ( bUngag ? "^n\y7.\r Flags: \d%s" : "^n\y7.\r Flags:\w %s" ) : ( bUngag ? "^n#. Flags: %s" : "^n7. Flags: %s" ), szFlags );
	...
	if( !bUngag )
	{
		...
		if( iGagTime )
		{
			...
			iLen += formatex( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "^n\y8.\r Time:\w %s^n" : "^n8. Time: %s^n", szTime );
		}
		else
			iLen += copy( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "^n\y8.\r Time:\y Permanent^n" : "^n8. Time: Permanent^n" );
	}
	else
		iLen += copy( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "^n\y8.\r UnGag^n" : "^n8. Ungag^n" );
	
	if( iEnd != iNum )
	{
		formatex( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "^n\y9.\w More..^n\y0.\w %s" : "^n9. More...^n0. %s", iPosition ? "Back" : "Exit" );
		iKeys |= MENU_KEY_9;
	}
	else
		formatex( szMenu[ iLen ], 511 - iLen, g_bColorSupported ? "^n\y0.\w %s" : "^n0. %s", iPosition ? "Back" : "Exit" );
...
{
	new szMenu[ 512 ];
	new iLen = copy( szMenu, 511, g_bColorSupported ? "\d[\yGeek~Gamers\d] \rGag Flags:^n^n" : "Gag Flags^n^n" );
	...
	if( g_bColorSupported )
	{
		...
		iLen += formatex( szMenu[ iLen ], 511 - iLen, "\r3.\w Voice: %s^n^n^n^n", ( g_iMenuFlags[ id ] & GAG_VOICE ) ? "\yYES" : "\rNO" );
	}
	else
	{
		...
		iLen += formatex( szMenu[ iLen ], 511 - iLen, "3. Voice: %s^n^n^n^n", ( g_iMenuFlags[ id ] & GAG_VOICE ) ? "YES" : "NO" );
	}
