...
public plugin_init()
{
	...
	register_cvar("gg_connect_string","%name (%steamid) Has Connected (%country).")
	register_cvar("gg_disconnect_string","%name (%steamid) Has Disconnected (%country).")
...
public client_putinserver(id)
{
	if(!is_user_bot(id))
	{
		...
		{
			new string[200]
			get_cvar_string("gg_connect_string",string,199)
			format(string,199,"^x04[GG] ^x01%s",string)
...
public client_disconnect(id)
{
	if(!is_user_bot(id))
	{
		...
		{
			new string[200]
			get_cvar_string("gg_disconnect_string",string,199)
			format(string,199,"^x04[GG] ^x01%s",string)
