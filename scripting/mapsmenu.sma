...
public plugin_init()
{
	...
	register_menucmd(register_menuid("\d[\yGeek~Gamers\d] \rChange Map Menu:"), 1023, "actionMapsMenu")
	...
	register_menucmd(register_menuid("\d[\yGeek~Gamers\d] \rVote Map Menu:"), 1023, "actionVoteMapMenu")
...
public checkVotes(id)
{
	...
	if (g_choosed != -1)
	{
		if (is_user_connected(id))
		{
			...
			format(menuBody[len], charsmax(menuBody) - len, "^n\y1. \w%L^n\y2. \w%L", id, "YES", id, "NO")
...
displayVoteMapsMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (g_voteSelectedNum[id] == 4 || isMapSelected(id, pos * 7 + b))
		{
			++b
			if (g_coloredMenus)
				len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%s^n\w", b, tempMap)
			else
				len += format(menuBody[len], charsmax(menuBody) - len, "#. %s^n", tempMap)
		} else {
			keys |= (1<<b)
			len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", ++b, tempMap)
		}
	}
	...
	{
		keys |= MENU_KEY_8
		len += format(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "START_VOT")
	}
	else
		len += format(menuBody[len], charsmax(menuBody) - len, g_coloredMenus ? "^n\d\y8. \w%L^n\w" : "^n#. %L^n", id, "START_VOT")

	if (end != g_mapNums)
	{
		len += format(menuBody[len], charsmax(menuBody) - len, "^n\y9. \w%L...^n\y0. \w%L^n", id, "MORE", id, pos ? "BACK" : "EXIT")
		keys |= MENU_KEY_9
	}
	else
		len += format(menuBody[len], charsmax(menuBody) - len, "^n\y0. \w%L^n", id, pos ? "BACK" : "EXIT")
	...
	switch (key)
	{
		case 7:
		{
			...
			if (g_voteSelectedNum[id] > 1)
			{
				...
				{
					ArrayGetString(g_mapName, g_voteSelected[id][c], tempMap, charsmax(tempMap));
					len += format(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", c + 1, tempMap)
					keys |= (1<<c)
				}
				
				keys |= (1<<8)
				len += format(menuBody[len], charsmax(menuBody) - len, "^n\y9. \w%L^n", id, "NONE")
			} else {
				ArrayGetString(g_mapName, g_voteSelected[id][0], tempMap, charsmax(tempMap));
				len = format(menuBody, charsmax(menuBody), g_coloredMenus ? "\y%L^n%s?^n\w^n\y1. \w%L^n\y2. \w%L^n" : "%L^n%s?^n^n\y1. \w%L^n\y2. \w%L^n", id, "CHANGE_MAP_TO", tempMap, id, "YES", id, "NO")
			...
			format(menuBody[len], charsmax(menuBody), "^n\y0. \w%L", id, "CANC_VOTE")
			keys |= MENU_KEY_0
