...
public plugin_init()
{
    register_clcmd("say /superknifex2","givesuperx2")
    register_clcmd("say /superx2","givesuperx2")
    register_clcmd("say /knifex2","givesuperx2")
    register_clcmd("say /x2","givesuperx2")
    register_clcmd("say superknifex2","givesuperx2")
    register_clcmd("say superx2","givesuperx2")
    register_clcmd("say knifex2","givesuperx2")
    register_clcmd("say x2","givesuperx2")
...
public plugin_precache()
{
	...
	format(szConfigFile, charsmax(szConfigFile), "%s/furien/items/superknifex2.ini", szConfigFile);
...
public givesuperx2(id)
{ 
	if(!(get_user_flags(id) & ADMIN_LEVEL)) return PLUGIN_HANDLED

	new Menu = menu_create("\d[\yGeek~Gamers\d] \rSuper Knife x2 \yMenu:", "Handlegivebow")
	
	menu_additem(Menu, "\wGet \rSuper Knife \yx2^n"   , "1", ADMIN_LEVEL)
	menu_additem(Menu, "\wGive \rSuper Knife \yx2 \wTo 1 \rFurien"   , "2", ADMIN_LEVEL)
	menu_additem(Menu, "\wGive \rSuper Knife \yx2 \wTo All \rFuriens"   , "3", ADMIN_LEVEL)
...
public Handlegivebow(id,menu,item){
	...
	switch(key)
	{
		...
		if(cs_get_user_team(id) == CS_TEAM_T)
			...
			ChatColor(id, "!t[GG] !nOnly !gFuriens !nCan Get a !gSuper Knife [x2]");
		}
		case 2:
		{
			menusuperx2(id)	
		...
		case 3:
		{
			...
			ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Give Super Knife [x2] To All ^3Furiens", szName)
		}
	}
	menu_destroy(menu);
	return PLUGIN_HANDLED
...
public menusuperx2(id)
{
	new GiveSuperx2 = menu_create ("\d[\yGeek~Gamers\d] \rGive \ySuper Knife x2:", "Handlemenusuperx2")
	...
	for (new i = 0; i < num; i++)
	{
		...
		menu_additem(GiveSuperx2, textmenu, szTempID, 0)
	}
	menu_display (id, GiveSuperx2)
	return PLUGIN_HANDLED
}

public Handlemenusuperx2(id, menu, item)
{
	if(item == MENU_EXIT)
	{
		menu_destroy(menu)
	...
	if(HasUserSuperKnife(tempid))
	{
		...
		ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Remove Super Knife [x2] To ^3%s", szName, szPlayerName)
	}
	else
	{
		SetUserSuperKnife(tempid)
		ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Give Super Knife [x2] To ^3%s", szName, szPlayerName)
	}
