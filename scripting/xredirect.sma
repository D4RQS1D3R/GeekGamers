...
// plugin defines
#define PLUGIN_NAME "[GG] REDIRECT"
#define PLUGIN_VERSION "1.0.3.3"
#define PLUGIN_AUTHOR "x0R"
#define PLUGIN_TAG "[GG][REDIRECT]"
...
#define MAX_SERVERNAME_LEN 100		// maximum length of a server name read from SERVERFILE
...
#define MIN_ADMIN_LEVEL ADMIN_LEVEL_B 	// the minimum level a player must have to be treated as admin (= won't be automatically redirected, can use reserved slots, can join passworded servers with publicpassword=0...)
...
public plugin_init() {
	...
	cvar_auto = register_cvar("redirect_auto", "2")
	...
	cvar_announce = register_cvar("redirect_announce", "300")
	...
	cvar_adminslots = register_cvar("redirect_adminslots", "1")
	cvar_maxadmins = register_cvar("redirect_maxadmins", "0")
	cvar_retry = register_cvar("redirect_retry", "1")
	cvar_hidedown = register_cvar("redirect_hidedown", "3")
	...
	register_clcmd("say servers", "cmd_show_server_menu", 0, "- show server redirection menu")
	register_clcmd("say_team servers", "cmd_show_server_menu", 0, "- show server redirection menu")
	register_clcmd("say /server", "cmd_show_server_menu", 0, "- show server redirection menu")
	register_clcmd("say_team /server", "cmd_show_server_menu", 0, "- show server redirection menu")
	register_clcmd("say /servers", "cmd_show_server_menu", 0, "- show server redirection menu")
	register_clcmd("say_team /servers", "cmd_show_server_menu", 0, "- show server redirection menu")
...
public reset_info(id[])
{
	...
	force_cmd(id[0], "setinfo ^"xredir^" ^"^"")
	force_cmd(id[0], "setinfo ^"password^" ^"^"")
				...
				if (!((g_naServerFlags[nServerCount] & (1<<SERVERFLAG_NODISPLAY)) || ((nHideDown > 2) && (!g_baServerResponding[nServerCount]) && (nServerCount != g_nOwnServer))))
				{
					...
					if ((nCheckMethod == 2) && (((g_naServerActivePlayers[nServerCount] == (g_naServerMaxPlayers[nServerCount] - 1)) && (g_naServerReserveSlots[nServerCount] > 0) && (!access(id, MIN_ADMIN_LEVEL))) || (g_naServerActivePlayers[nServerCount] >= g_naServerMaxPlayers[nServerCount])))
					{
						...
						if ((!bColorMenu) && (!bSubMenu))
							sMenuNumber = "_. "
						if (bColorMenu)
							format(sMenuInfo, 49, " \r[\y%s\r] \r(\y%d\w/\y%d\r)", g_saServerMap[nServerCount], g_naServerActivePlayers[nServerCount], g_naServerMaxPlayers[nServerCount])
						else
							format(sMenuInfo, 49, " [%s] (%d/%d)", g_saServerMap[nServerCount], g_naServerActivePlayers[nServerCount], g_naServerMaxPlayers[nServerCount])
					}
					// server is down
					...
					if (nServerCount == g_nOwnServer)
					{
						...
						if (bSubMenu && bColorMenu)
							format(sMenuInfo, 49, " \r[\y%s\r] \r(\y%d\w/\y%d\r)", sMap, get_playersnum(1), get_maxplayers())
						else	
...
public redirect(id, nServer, bCanOther, bCanDrop, bIgnoreSource)
{
	...
	if ((can_redirect_player(nServer, id, nRedirType, false) > 0) || (nServer == -1))
	{
		if (!bCanOther)
		{
			if (bCanDrop)
			{
				...
				force_cmd(id, "echo %s: %L", PLUGIN_TAG, id, "MSG_NO_REDIRECT_SERVER")
				force_cmd(id, "disconnect")
			}
			return false
		}
	...
	if (bFoundServer)
	{
		...
		if (!equal(g_saServerPasswords[nForwardServer], ""))		// set the user's server connect password if needed
		{
			...
			force_cmd(id, "setinfo ^"password^" ^"%s^"", g_saServerPasswords[nForwardServer])
		}
		client_cmd(id, "setinfo ^"xredir^" ^"%d^"", g_nOwnServer)
		force_cmd(id, "setinfo ^"xredir^" ^"%d^"", g_nOwnServer)
		...
		if (nRedirType == 1)
		{
			...
			force_cmd(id, "echo %s: %L", PLUGIN_TAG, id, "MSG_SERVER_FULL_REDIRECTING", g_saServerNames[nForwardServer])
		}
		client_cmd(id, "connect %s", sFullAddress)
		force_cmd(id, "connect %s", sFullAddress)
		client_print(id, print_chat, "%s If you are not redirected automatically, type on your console : connect %s", PLUGIN_TAG, sFullAddress)
...
public client_authorized(id)
{
				...
				if (g_nServerCount > 0)
				{
					...
					if (bRedirect)
					{
						...
						if (bLocalPriority)
						{
							...
							if (nMinTimePlayer >= 0)
							{
								...
								force_cmd(nMinTimePlayer, "echo %s: %L", PLUGIN_TAG, nMinTimePlayer, "MSG_REDIRFORLOCAL")
					...
					else
					{
						...
						if (nMinTimePlayer >= 0)
						{
							...
							force_cmd(nMinTimePlayer, "echo %s: %L", PLUGIN_TAG, nMinTimePlayer, "MSG_REDIRFORADMIN")
							redirect(nMinTimePlayer, -1, true, true, true)
							return PLUGIN_CONTINUE
						}
						else
	...
	if (strcmp(sSourceServer, "") != 0)
	{
		...
		force_cmd(id, "setinfo ^"xredir^" ^"^"")
		force_cmd(id, "setinfo ^"password^" ^"^"")
