...
public plugin_init()
{
	...
	cvarmenu_cmdid=register_clcmd("ggsv_amx_plugincvarmenu", "CvarMenuCommand", ADMIN_CVAR, " - displays the plugin cvar menu");
	cmdmenu_cmdid=register_clcmd("ggsv_amx_plugincmdmenu", "CommandMenuCommand", ADMIN_MENU, " - displays the plugin command menu");
	
	register_clcmd("ggsv_amx_changecvar","CommandChangeCvar");
	register_clcmd("ggsv_amx_executecmd","CommandExecuteCommand");
...
public addToMenuFront()
{
	...
	if (strcmp(cmd, "ggsv_amx_plugincmdmenu") != 0)
	{
	...
	if (strcmp(cmd, "ggsv_amx_plugincvarmenu") != 0)
	{
		// this should never happen, but just incase!
		cvarflags = ADMIN_CVAR;
	}

	AddMenuItem("Plugin Cvars", "ggsv_amx_plugincvarmenu", cvarflags, PluginFileName);
	AddMenuItem("Plugin Commands", "ggsv_amx_plugincmdmenu", cmdflags, PluginFileName);
}
...
public CvarMenuSelection(id, menu, item)
{
	...
	else
	{
		...
		client_cmd(id,"messagemode ggsv_amx_changecvar");
...
/**
 * Process the "ggsv_amx_plugincvarmenu" command.
 *
...
public SpecificCommandHandler(id,menu,item)
{
	...
	if (item==0)  // "With params"
	{
		...
		client_cmd(id,"messagemode ggsv_amx_executecmd");
...
/**
 * Handles the executed command (via "ggsv_amx_executecmd").
 *
...
public CommandMenuSelection(id, menu, item)
{
	if (item==MENU_EXIT)
	{
		...
		if (ExplicitPlugin[id]==-1)
		{
			client_cmd(id,"ggsv_amx_plugincmdmenu");
		}
	}
	else if (item==MENU_BACK)
	{
...
/**
 * Handles the "ggsv_amx_plugincmdmenu" command.
 *
