...
public plugin_init()
{
	...
	register_clcmd("ggsv_amx_clcmdmenu", "cmdClcmdMenu", ADMIN_LEVEL_A, "- displays client cmds menu");
...
public plugin_cfg()
{
	...
	if (!p_amx_tempban_maxtime)
	{
		...
		server_cmd("ggsv_amx_cvar add %s", amx_tempban_maxtime);
	}
}
...
displayBanMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (is_user_bot(i) || (access(i, ADMIN_IMMUNITY) && i != id))
		{
			...
			if (g_coloredMenus)
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%s^n\w", b, name);
			}
			else
			{
		...
		else
		{
			...
			if (is_user_admin(i))
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, g_coloredMenus ? "\y%d. \w%s \r*^n\w" : "\y%d. \w%s *^n", ++b, name);
			}
			else
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", ++b, name);
			}
		}
	}
	...
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "BAN_FOR_MIN", g_menuSettings[id]);
	}
	else
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "BAN_PERM");
	}

	if (end != g_menuPlayersNum[id])
	{
		formatex(menuBody[len], charsmax(menuBody) - len, "^n\y9. \w%L...^n\y0. \w%L", id, "MORE", id, pos ? "BACK" : "EXIT");
		keys |= MENU_KEY_9;
	}
	else
	{
		formatex(menuBody[len], charsmax(menuBody) - len, "^n\y0. \w%L", id, pos ? "BACK" : "EXIT");
	}
...
displaySlapMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (!is_user_alive(i) || (access(i, ADMIN_IMMUNITY) && i != id))
		{
			...
			if (g_coloredMenus)
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \d%s\R%s^n\w", b, name, team);
			}
			else
			{
		...
		else
		{
			...
			if (is_user_admin(i))
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, g_coloredMenus ? "\y%d. \w%s \r*\y\R%s^n\w" : "\y%d. \w%s *   %s^n", ++b, name, team);
			}
			else
			{
				len += formatex(menuBody[len], charsmax(menuBody) - len, g_coloredMenus ? "\y%d. \w%s\y\R%s^n\w" : "\y%d. \w%s   %s^n", ++b, name, team);
			}
		}
	}
	...
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "SLAY");
	}
	else
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "SLAP_WITH_DMG", g_menuSettings[id]);
	}
...
displayTeamMenu(id, pos)
{
	...
	len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y7. \r%L: \y%L", id, "TRANSF_SILENT", id, g_silent[id] ? "YES" : "NO");
	len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \r%L^n", id, "TRANSF_TO", g_CSTeamNames[g_menuOption[id] % 3]);
...
displayClcmdMenu(id, pos)
{
	...
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%s^n", g_clcmdName[g_menuSelect[id][g_menuOption[id]]]);
	}
	else
	{
		len += formatex(menuBody[len], charsmax(menuBody) - len, "^n\y8. \w%L^n", id, "NO_CMDS");
	}

	if (end != g_menuPlayersNum[id])
	{
		formatex(menuBody[len], charsmax(menuBody) - len, "^n\y9. \w%L...^n\y0. %L", id, "MORE", id, pos ? "BACK" : "EXIT");
		keys |= MENU_KEY_9;
	}
	else
	{
