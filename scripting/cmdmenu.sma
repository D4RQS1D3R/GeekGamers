...
public plugin_init()
{
	...
	register_clcmd("ggsv_amx_cvarmenu", "cmdCvarMenu", ADMIN_CVAR, "- displays cvars menu");
...
displayCmdMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		{
			keys |= (1<<b);
			len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s^n", ++b, g_cmdName[g_menuSelect[id][a]]);
		}
	}
...
displayCvarMenu(id, pos)
{
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (g_coloredMenus)
		{
			len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s\R%s^n\w", b, g_cvarNames[g_menuSelect[id][a]], szValue);
		}
		else
		{
			len += formatex(menuBody[len], charsmax(menuBody) - len, "\y%d. \w%s    %s^n", b, g_cvarNames[g_menuSelect[id][a]], szValue);
		}
	}
