...
public plugin_init()
{
	...
	register_concmd("ggsv_amx_reloadadmins", "cmdReload", ADMIN_CFG)
	//register_concmd("ggsv_amx_addadmin", "addadminfn", ADMIN_RCON, "<playername|auth> <accessflags> [password] [authtype] - add specified player as an admin to users.ini")
	...
	loadSettings(configsDir2)					// Load admins accounts

	format(configsDir3, 63, "%s/manager/users.ini", configsDir3)
	loadSettings(configsDir3)					// Load admins accounts
#endif
}
...
#if !defined USING_SQL
	new filename[128], filename2[128], filename3[128]
	
	get_configsdir(filename, charsmax(filename))
	format(filename, charsmax(filename), "%s/users.ini", filename)
	
	get_configsdir(filename2, charsmax(filename2))
	format(filename2, charsmax(filename2), "%s/auto-admins.ini", filename2)
	
	get_configsdir(filename3, charsmax(filename3))
	format(filename3, charsmax(filename3), "%s/manager/users.ini", filename3)

	AdminCount = 0;
	loadSettings(filename);		// Re-Load admins accounts
	loadSettings(filename2);	// Re-Load admins accounts
	loadSettings(filename3);	// Re-Load admins accounts
...
getAccess2(id, name[], authid[], ip[], password[])
{
	new index = -1
	new result = 0
...
accessUser2(id, name[] = "")
{
	...
	new result = getAccess2(id, username, userauthid, userip, password)
