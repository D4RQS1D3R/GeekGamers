...
#define VERSION "1.0"
#define AUTHOR "Dias Leon" // Edited By ~DarkSiDeRs~

// =========== Resources Config
#define V_MODEL "models/[GeekGamers]/Primary/v_bow2.mdl"
#define P_MODEL "models/[GeekGamers]/Primary/p_bow.mdl"
#define P_MODEL_EMPTY "models/[GeekGamers]/Primary/p_bow_empty.mdl"
#define W_MODEL "models/[GeekGamers]/Primary/w_bow.mdl"

#define ARROW_MODEl "models/[GeekGamers]/Primary/arrow.mdl"
...
public plugin_init()
{
	...
	register_clcmd("say /compoundbow", "GiveBow")
	register_clcmd("say /compound", "GiveBow")
	register_clcmd("say /bow", "GiveBow")
	register_clcmd("say /cb", "GiveBow")
	register_clcmd("say compoundbow", "GiveBow")
	register_clcmd("say compound", "GiveBow")
	register_clcmd("say bow", "GiveBow")
	register_clcmd("say cb", "GiveBow")
...
{
	register_native("gg_get_user_compoundbow", "get_user_compoundbow", 1)
	register_native("gg_set_user_compoundbow", "set_user_compoundbow")
}
...
{ 
	...
	new Menu = menu_create("\d[\yGeek~Gamers\d] \rCompound Bow \yMenu:", "HandleGiveBow")
	
	menu_additem(Menu, "\wGet \rCompound \yBow^n"   , "1", ADMIN_LEVEL_C)
	menu_additem(Menu, "\wGive \rCompound \yBow \wTo 1 \rFurien"   , "2", ADMIN_LEVEL_C)
	menu_additem(Menu, "\wGive \rCompound \yBow \wTo All \rFuriens"   , "3", ADMIN_LEVEL_C)
	...
	switch(key)
	{
		case 1:
		{
			if(cs_get_user_team(id) == CS_TEAM_T)
				Get_CompoundBow(id)
			else ColorChat(id, RED, "^3[GG] ^1Only ^4Furiens ^1Can Get a ^4Compound Bow ^1!")
		}
		case 2: GiveBow1(id)
		case 3:
		{
   			...
   			for (new i = 0; i < num; i++)
			{
				...
				if(!g_have_bow[tempid])
					Get_CompoundBow(tempid)
			}
			ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Give Compound Bow To All ^3Furiens", szName)
		}
	}
...
public GiveBow1(id)
{
	new menu = menu_create ("\d[\yGeek~Gamers\d] \rGive \yCompound Bow:", "HandleGiveBow1")
	...
	for (new i = 0; i < num; i++)
	{
		...
		else formatex(textmenu, 63, "%s \y- \r[OFF]", szName)
...
public HandleGiveBow1(id, menu, item)
{
	if(item == MENU_EXIT)
	{
		menu_destroy(menu)
	...
	if( g_have_bow[tempid] )
	{
		Remove_CompoundBow(tempid)
		ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Remove The Compound Bow of ^3%s", szName, szPlayerName)
	}
	else
	{
		Get_CompoundBow(tempid)
		ColorChat(0, RED, "^4[GG] ^1OWNER ^4%s ^1: Give Compound Bow To ^3%s", szName, szPlayerName)
	}
...
public set_user_compoundbow(iPlugin, iParams)
{
	new id = get_param(1)

	Set_BitVar(g_Had_CompoundBow, id)
	g_BowArrow[id] = max(0, get_param(2))
	g_WeaponState[id] = WEAPON_NONE
	g_have_bow[id] = true
	...
	return g_BowArrow[id]
}

public get_user_compoundbow(id)
{
	return g_have_bow[id]
}
...
public HAM_Spawn_Post(id)
{
	...
	if(cs_get_user_team(id) == CS_TEAM_T && g_have_bow[id])
	{
		new g_OldBowArrow = g_BowArrow[id]
		Get_CompoundBow(id)
		g_BowArrow[id] = g_OldBowArrow
		UpdateAmmo(id, CSW_BOW, BOW_AMMOID, -1, g_BowArrow[id])
	}
}
...
public Event_DeathMsg()
{
	...
	set_task(1.0, "Remove_CompoundBow", iVictim)
}
